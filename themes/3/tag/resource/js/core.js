jQuery.extend({
		...
		if (e.type == 'click') {
			...
			if (has_meta && $('input.cm-item[type=checkbox]', elm.form).length > 0) { // check if we have such elms in the form only
				...
				if (jelm.hasClass('cm-confirm') || jelm.parents().hasClass('cm-confirm')) {
					if (jelm.hasClass('cm-confirm-order')) {
						if (confirm("Chức năng xác nhận nhiều đơn hàng này dành cho các khách hàng cũ. Bạn có chắc chắn muốn xác nhận những đơn hàng này không?") == false) {
							return false;
						}
					} else {
						if (confirm(lang.text_are_you_sure_to_proceed) == false) {
			...
			if (jelm.hasClass('cm-dialog-opener') || jelm.parents('.cm-dialog-opener').length) {
				...
				if (_e.attr("dgWidth") && _e.attr("dgHeight")) {
					$('#' + _e.attr('rev')).ceDialog('open', {width: jelm.attr("dgWidth"), height: jelm.attr("dgHeight"), href: _e.attr('href'), keepInPlace: _e.hasClass('cm-dialog-keep-in-place')});
				} else {
	...
	processForms: function(elm)
	{
				...
				if (section) {
					...
					$('select#' + $('.cm-country.cm-location-' + section).attr('for')).change(function() {
						...
						if (label.attr('class')) {
							...
							var section = location_elm ? location_elm[1] : '';
							if (section) {
								jQuery.profiles.rebuild_states(section, states_elm, true);
							}
						}
					});
				}
		...
		$('label.cm-district', elm).each(function() {
			var label = $(this);
			if (label.attr('class')) {
				var location_elm = label.attr('class').match(/cm-location-([^\s]+)/i);
				var section = location_elm ? location_elm[1] : '';
				if (section) {
					var districts_elm = $('#' + label.attr('for')).attr('id');
					// jQuery.profiles.rebuild_districts(section, districts_elm);
		...
		$('label.cm-ward', elm).each(function() {
			var label = $(this);
			if (label.attr('class')) {
				var location_elm = label.attr('class').match(/cm-location-([^\s]+)/i);
				var section = location_elm ? location_elm[1] : '';
				if (section) {
					var wards_elm = $('#' + label.attr('for')).attr('id');
					// jQuery.profiles.rebuild_wards(section, wards_elm);
					$('select#' + $('.cm-district.cm-location-' + section).attr('for')).change(function() {
						var label = $('label[for=' + $(this).attr('id') + ']');
						if (label.attr('class')) {
							var location_elm = label.attr('class').match(/cm-location-([^\s]+)/i);
							var section = location_elm ? location_elm[1] : '';
...
function form_handler(form)
{
	...
	this.check_fields = function()
	{
		...
		for (i=0; i < elms.length; i++) {
			...
			// Bug #1057 - Cập nhật rule Ngày hết hạn giao hàng trong Chi tiết deal ở BE
			//604800: Thời gian 1 tuần
			if (lbl.hasClass('cm-expire-delivery')) {
				var date_avail_to_holder;
				var date_delivery_time_holder;
				var dt = $("input#date_avail_to_holder").datepicker('getDate');
				var d = new Date(dt);
				date_avail_to_holder = d.getTime() / 1000;
				
				dt = $("input#date_delivery_time_holder").datepicker('getDate');
				d = new Date(dt);
				date_delivery_time_holder = d.getTime() / 1000;
				if( (date_delivery_time_holder - date_avail_to_holder) < 604800)
				{
					is_ok = false;
					set_mark = true;
					this.form_message('Ngày hết hạn giao hàng phải lớn hơn Ngày hết hạn Deal tối thiểu là 7 ngày', lbl);
				}
	...
	this.check = function()
	{
		...
		if (check_fields_result == true && form_result == true) {
			...
			$('.btn-hidden-success input[type=submit]').hide();
